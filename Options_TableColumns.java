...
**/
  public void buildObject(XMLNode node){
    ...
    XMLNode srcNode = XMLReader.read(new File("settings" + File.separator + "interface_SidePanel_Options_TableColumns.wsd"));
...
**/
  public void loadColumns(){
    ...
    WSPanel checkboxHolder = (WSPanel)WSRepository.get("SidePanel_Options_TableColumns_Checkboxes");
    checkboxHolder.removeAll();
    checkboxHolder.setLayout(new GridLayout(1,columns.length));

    java.util.Arrays.sort(columns);

    checkboxHolder.setLayout(new GridLayout(columns.length,1));

    for (int i=0;i<columns.length;i++){
      String columnCode = columns[i].getCode();
      checkboxHolder.add(WSHelper.buildComponent(XMLReader.readString("<WSOptionCheckBox code=\"Option_TableColumn_" + columnCode + "\" setting=\"ShowTableColumn_" + columnCode + "\" />")));
      ...
      String langCode = "WSCheckBox_ShowTableColumn_" + columnCode + "_Text";
      if (! Language.has(langCode,false)){
        Language.set(langCode,columns[i].getName());
        }

      Language.set("WSCheckBox_ShowTableColumn_" + columnCode + "_Tooltip","Show this column in the TextList Table.");
      }
    }
...
**/
  public void sortCheckboxes(){
    WSPanel panel = (WSPanel)WSRepository.get("SidePanel_Options_TableColumns_Checkboxes");
