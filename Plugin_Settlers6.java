...
**********************************************************************************************
**/
public class Plugin_Settlers6 extends ArchivePlugin {
...
**********************************************************************************************
**/
  public Plugin_Settlers6() {

    super("Settlers6","Settlers6");

    //           read write replace
    setProperties(true,true,true);
    setAllowsUnicode(false);

    setGames("Settlers 6: Rise Of An Empire");
    setExtensions("de","es","fr","it","nl","uk","us"); // MUST BE LOWER CASE
    setPlatforms("PC");
  ...
  public Resource[] read(File path) {
    try {
      ...
      while (fm.getFilePointer() < arcSize){
        ...
        if (tabPos <= 0 || equalPos < tabPos){
          // bad line
          continue;
          }

        String code = line.substring(0,tabPos);
        String original = line.substring(equalPos+2);

        //original,code
        resources[realNumFiles] = new Resource_Settlers6(original,code);
  ...
  public void write(Resource[] resources, File path) {
    try {
      ...
      for (int i=0;i<numFiles;i++){
        ...
        if (text instanceof Resource_Settlers6){
          Resource_Settlers6 resource = (Resource_Settlers6)text;
